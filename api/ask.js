...
Specia curenta: ${species || 'general (toate speciile)'}`;
    ...
    // Incearca mai intai modelul rapid (8b), apoi cel complet (70b) ca fallback
    ...
    for (const model of MODELS) {
      ...
      try {
        ...
        if (groqRes.status === 429) { lastErr = new Error('rate_limit'); continue; }
        ...
        clearTimeout(fetchTimer);
        if (fetchErr.name === 'AbortError') { lastErr = fetchErr; continue; }
        throw fetchErr;
      }
    }

    // Ambele modele au esuat
    return Response.json(
      { error: lastErr?.name === 'AbortError' ? 'AI-ul raspunde lent (suprasolicitat). Incearca din nou in cateva secunde.' : 'Serviciul AI indisponibil temporar. Incearca din nou.' },
