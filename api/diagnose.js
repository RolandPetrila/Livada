...
Fii concis, practic, cu informatii pe care un pomicultor le poate aplica imediat.`;
  ...
  const [geminiSettled, plantIdSettled] = await Promise.allSettled([
    callGemini(GEMINI_KEY1, 'gemini-2.5-flash', base64, mimeType, prompt, 20000),
    PLANT_ID_KEY
  ...
  if (plantIdSettled.status === 'fulfilled' && plantIdSettled.value.ok) {
    ...
    } catch { log('plant.id parse err'); }
  } else {
    log(`plant.id skip: ${plantIdSettled.reason?.message || plantIdSettled.value?.status}`);
  }
  ...
  if (geminiSettled.status === 'fulfilled' && geminiSettled.value.ok) {
    try {
      ...
      if (text) {
        ...
        return Response.json({ diagnosis: finalText, ...meta }, { headers: corsHeaders(req) });
      }
    } catch { log('gemini parse err'); }
  } else {
    log(`gemini-2.5-flash key1 failed: ${geminiSettled.reason?.name || geminiSettled.value?.status}`);
  ...
  if (plantIdPrefix) {
    ...
    return Response.json({
      diagnosis: plantIdPrefix + '\n_Analiza AI detaliata indisponibila momentan. Plant.id a detectat bolile de mai sus._',
      _fallback: true, _fallbackModel: 'plant.id', _plantid: true,
    }, { headers: corsHeaders(req) });
  }

  // ── Fallback 2: Gemini 2.5-flash-lite cheia 1 ────────────────────────────────
  try {
    const res = await callGemini(GEMINI_KEY1, 'gemini-2.5-flash-lite', base64, mimeType, prompt, 15000);
  ...
  } catch (e) { log(`flash-lite err: ${e.name}`); }

  // ── Fallback 3: Gemini 2.5-flash cheia 2 ─────────────────────────────────────
  const GEMINI_KEY2 = process.env.GOOGLE_AI_API_KEY_2;
  if (GEMINI_KEY2) {
    try {
      const res = await callGemini(GEMINI_KEY2, 'gemini-2.5-flash', base64, mimeType, prompt, 15000);
